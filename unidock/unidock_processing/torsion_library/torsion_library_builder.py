...
class TorsionLibraryBuilder(object):
    def __init__(self, working_dir_name):
        ...
        self.torsion_library_xml_file_name = os.path.join(os.path.dirname(__file__), 'data', 'torsion_library_2020.xml')
        self.working_dir_name = os.path.abspath(working_dir_name)
        self.torsion_library_pickle_file_name = os.path.join(self.working_dir_name, 'torsion_library.pkl')
    ...
    def build_torsion_library(self):
        ...
        for hierarchy_class_node in hierarchy_class_node_list:
            ...
            hierarchy_class_atom_map_dict = utils.get_pattern_atom_mapping(hierarchy_class_pattern_mol)
            ...
            if hierarchy_class_name == 'GG':
                ...
                torsion_library_dict['generic_class_pattern_mol'] = hierarchy_class_pattern_mol
                torsion_library_dict['generic_class_atom_map_dict'] = hierarchy_class_atom_map_dict
                torsion_library_dict['generic_class_node'] = hierarchy_class_node

            else:
                torsion_library_dict['specific_class_name_list'].append(hierarchy_class_name)
                torsion_library_dict['specific_class_smarts_list'].append(hierarchy_class_smarts)
                torsion_library_dict['specific_class_pattern_mol_list'].append(hierarchy_class_pattern_mol)
                torsion_library_dict['specific_class_atom_map_dict_list'].append(hierarchy_class_atom_map_dict)
                torsion_library_dict['specific_class_node_list'].append(hierarchy_class_node)
        ...
        torsion_library_dict['specific_class_node_info_list'] = [None] * num_specific_class

        for class_idx in range(num_specific_class):
            self.node_idx = 0
            torsion_class_node = torsion_library_dict['specific_class_node_list'][class_idx]
            ...
            torsion_class_info_dict['pattern_mol'] = Chem.MolFromSmarts(torsion_class_node.get('smarts'))
            torsion_class_info_dict['atom_map_dict'] = utils.get_pattern_atom_mapping(torsion_class_info_dict['pattern_mol'])
            ...
            torsion_library_dict['specific_class_node_info_list'][class_idx] = torsion_class_info_dict
    ...
    def __collect_torsion_rules__(self, node, parent_node_info_dict):
        ...
        if node.tag == 'hierarchySubClass':
            ...
            node_info_dict['atom_map_dict'] = utils.get_pattern_atom_mapping(node_info_dict['pattern_mol'])
