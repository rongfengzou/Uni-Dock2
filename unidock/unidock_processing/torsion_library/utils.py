...
def get_detailed_torsion_ranges(torsion_range_list):
    ...
    for torsion_range_tuple in torsion_range_list:
        ...
        if torsion_lower_bound > torsion_upper_bound:
            detailed_torsion_tuple_list = [(torsion_lower_bound, 180.0), (-180.0, torsion_upper_bound)]
    ...
    for torsion_range in torsion_range_list:
        ...
        if len(merged_torsion_range_list) == 0:
            ...
            if checked_torsion_upper_bound < torsion_lower_bound:
                ...
                updated_torsion_range = (checked_torsion_lower_bound, max(checked_torsion_upper_bound, torsion_upper_bound))
...
def analyze_torsion_rule(torsion_rule_node):
    ...
    for torsion_angle_list_node in torsion_angle_list_node_list:
        ...
        if set({-150.0, -120.0, -90.0, -60.0, -30.0, 0.0, 30.0, 60.0, 90.0, 120.0, 150.0, 180.0}).issubset(torsion_value_set):
            ...
            for bin_idx in range(num_torsion_bins):
                ...
                torsion_lower_bound = canonicalize_angle(torsion_value - torsion_tolerance)
                torsion_upper_bound = canonicalize_angle(torsion_value + torsion_tolerance)

                torsion_distribution[bin_idx] = (torsion_lower_bound, torsion_upper_bound)
    ...
    for torsion_histogram_node in torsion_histogram_node_list:
        ...
        for bin_idx in range(num_weight_bins):
            ...
            torsion_histogram_weight_list[bin_idx] = float(histogram_weight_info['count'])
...
def get_torsion_mobile_atom_idx_list(mol, rotatable_bond_info, root_atom_idx):
    ...
    for bond in bond_list:
        ...
        if bond_info == rotatable_bond_info or bond_info_reversed == rotatable_bond_info:
...
def rotate_torsion_angle(mol, torsion_atom_idx_list, mobile_atom_idx_list, torsion_angle):
    ...
    target_torsion_value = np.degrees(calc_dihedrals(torsion_atom_position_0,
    ...
    unit_dihedral_rotate_axis = dihedral_rotate_axis / np.linalg.norm(dihedral_rotate_axis)

    delta_torsion_angle = torsion_angle - target_torsion_value
    delta_torsion_angle = np.radians(delta_torsion_angle)

    transformation_matrix = get_rotation_matrix(delta_torsion_angle, unit_dihedral_rotate_axis, torsion_bond_position_0)
    ...
    for atom_idx in range(num_total_atoms):
        ...
        atom_coord_point_3D = Point3D(atom_positions[0], atom_positions[1], atom_positions[2])
