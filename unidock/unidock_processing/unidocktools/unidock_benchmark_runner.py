...
class UnidockBenchmarkRunner(object):
    def __init__(self,
        ...
        self.crystal_mol = Chem.SDMolSupplier(self.ligand_crystal_sdf_file_name, removeHs=True)[0]
        self.atom_mapping_array = np.array(self.crystal_mol.GetSubstructMatches(self.crystal_mol, useChirality=True, uniquify=False))
    ...
    def __calculate_best_matched_rmsd__(crystal_coords_array, coords_array, atom_mapping_array):
        ...
        for atom_order_idx in range(num_atom_orders):
            ...
            rmsd = np.sqrt(np.sum((reordered_coords_array - crystal_coords_array)**2) / num_atoms)
            if rmsd < min_rmsd:
                min_rmsd = rmsd
    ...
    def run_unidock_benchmark(self):
        ...
        unidock_pose_writer = UnidockLigandPoseWriter(self.ligand_mol_list,
                                                      unidock2_pose_json_file_name_list,
                                                      self.working_dir_name)

        unidock_pose_writer.generate_docking_pose_sdf()

        unidock_pose_mol_list = list(Chem.SDMolSupplier(unidock_pose_writer.docking_pose_sdf_file_name, removeHs=True))
        ...
        for pose_idx in range(num_docked_poses):
            ...
            unidock_pose_rmsd = self.__calculate_best_matched_rmsd__(self.crystal_positions, unidock_pose_positions, self.atom_mapping_array)
