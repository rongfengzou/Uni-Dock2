...
class UnidockLigandPoseWriter(object):
    def __init__(self,
        ...
        for batch_idx in range(self.num_unidock2_batches):
            unidock2_pose_json_file_name = self.unidock2_pose_json_file_name_list[batch_idx]
    ...
    def generate_docking_pose_sdf(self):
        self.docking_pose_sdf_file_name = os.path.join(self.working_dir_name, 'unidock2_pose.sdf')
        ...
        for ligand_idx in range(self.num_ligands):
            ...
            for pose_idx in range(num_poses):
                ...
                ligand_mol_ud2_pose.SetDoubleProp('vina_binding_free_energy', float(vina_scoring_list[0]))
                ligand_mol_ud2_pose.SetDoubleProp('vina_intra_inter', float(vina_scoring_list[1]))
                ligand_mol_ud2_pose.SetDoubleProp('vina_intra', float(vina_scoring_list[2]))
                ligand_mol_ud2_pose.SetDoubleProp('vina_inter', float(vina_scoring_list[3]))
                ligand_mol_ud2_pose.SetDoubleProp('vina_box_penalty', float(vina_scoring_list[4]))
                ligand_mol_ud2_pose.SetDoubleProp('vina_torsion_number_energy', float(vina_scoring_list[5]))
                ...
                if num_coords_atoms != num_ligand_atoms:
                    raise ValueError(f'Unidock2 output poses do not have equal number of atoms compared to original molecule on {ligand_pose_name}!!')

                ud2_pose_coord_array = np.reshape(ud2_pose_coord_list, (num_ligand_atoms, 3))

                for atom_idx in range(num_ligand_atoms):
                    ud2_atom_coord = ud2_pose_coord_array[atom_idx, :]
                    ud2_atom_coord_point_3D = Point3D(float(ud2_atom_coord[0]), float(ud2_atom_coord[1]), float(ud2_atom_coord[2]))
                    ligand_mol_ud2_conf.SetAtomPosition(atom_idx, ud2_atom_coord_point_3D)
