...
def build_molecular_graph(mol,
    ...
    if template_docking:
        ...
        core_atom_idx_list = utils.get_core_alignment_for_template_docking(reference_mol, mol, core_atom_mapping_dict)

        ################################################################################################################
        ################################################################################################################
        ## The tail parts of atoms in the core are cancelled and not beloings to the core itself.
        ## Currently this strategy is disabled temporarily.
        ...
        temp_template_aligned_ligand_sdf_file_name = os.path.join(working_dir_name, 'ligand_template_aligned.sdf')
    ...
    atom_type_list, partial_charge_list, atom_parameter_dict, torsion_parameter_nested_dict = utils.record_gaff2_atom_types_and_parameters(temp_ligand_sdf_file_name, 'gas', working_dir_name)
    ###############################################################################
    ...
    torsion_library_driver = TorsionLibraryDriver(mol, rotatable_bond_info_list, torsion_library_dict)
    ...
    for torsion_idx in range(torsion_library_driver.num_rotatable_bonds):
        ...
        torsion_type = (torsion_atom_type_i, torsion_atom_type_j, torsion_atom_type_k, torsion_atom_type_l)
        ...
        torsion_parameter_dict_list = torsion_parameter_nested_dict[corrected_torsion_type]
        ...
        for torsion_parameter_idx in range(num_torsion_parameters):
            ...
            torsion_parameter_nested_list[torsion_parameter_idx] = torsion_parameter_list
    ...
    return atom_info_nested_list, torsion_info_nested_list, root_atom_idx_list

def batch_topology_builder_process(ligand_sdf_file_name,
                                   covalent_ligand,
                                   template_docking,
    ...
    for ligand_idx in range(num_batch_ligands):
        ...
        atom_info_nested_list, torsion_info_nested_list, root_atom_idx_list = build_molecular_graph(ligand_mol,
                                                                                                    torsion_library_dict,
                                                                                                    covalent_ligand,
                                                                                                    template_docking,
                                                                                                    reference_mol,
...
class UnidockLigandTopologyBuilder(object):
    def __init__(self,
        ...
        for ligand_sdf_file_name in self.ligand_sdf_file_name_list:
            ligand_mol_list = list(Chem.SDMolSupplier(ligand_sdf_file_name, removeHs=False))
            for ligand_mol in ligand_mol_list:
                if ligand_mol is None:
                    logging.error(f'Incorrect bond orders for molecule!')
                    continue
                if Descriptors.NumRadicalElectrons(ligand_mol) > 0:
                    logging.error(f'Molecule contains atoms with radicals!')
                    continue
        ...
        if self.num_ligands == 0:
            raise ValueError('Zero valid molecule after checking when preprocessing ligands!!')
        ...
        if len(self.core_atom_mapping_dict_list) != self.num_ligands:
            raise ValueError('Number of user specified core atom mapping dicts does not match the number of input molecules!!')
        ...
        self.ligand_json_file_name = os.path.join(self.root_working_dir_name, 'ligands_unidock2.json')
    ...
    def generate_batch_ligand_topology(self):
        ...
        for batch_idx in range(num_batches):
            ...
            batch_ligand_mol_list = self.ligand_mol_list[batch_ligand_idx_tuple[0]:batch_ligand_idx_tuple[1]]
            batch_core_atom_mapping_dict_list = self.core_atom_mapping_dict_list[batch_ligand_idx_tuple[0]:batch_ligand_idx_tuple[1]]
            ...
            batch_ligand_topology_builder_results = batch_ligand_topology_builder_pool.apply_async(batch_topology_builder_process, args=(batch_ligand_sdf_file_name,
        ...
        for batch_idx in range(num_batches):
            batch_ligand_topology_builder_results = batch_ligand_topology_builder_results_list[batch_idx]
        ...
        if len(self.total_ligand_info_dict_list) != self.num_ligands:
            raise ValueError('Collected number of batch ligands does not equal to real total number of input ligands!!')
    ...
    def get_summary_ligand_info_dict(self):
        ...
        for ligand_idx in range(self.num_ligands):
            ...
            self.total_ligand_info_summary_dict[ligand_name] = {'atoms': atom_info_nested_list, 'torsions': torsion_info_nested_list, 'root_atoms': root_atom_idx_list}
