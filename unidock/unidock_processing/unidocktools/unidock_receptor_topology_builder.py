...
class UnidockReceptorTopologyBuilder(object):
    def __init__(self,
        ...
        self.receptor_structure_dms_file_name = os.path.join(self.working_dir_name, 'receptor_structure.dms')
        self.receptor_parameterized_dms_file_name = os.path.join(self.working_dir_name, 'receptor_parameterized.dms')

    def run_protein_preparation(self):
        if which('fepfixer') is not None and which('utop') is not None:
            if self.prepared_hydrogen:
                fep_fixer_command = f'fepfixer -i {self.receptor_file_name} -o {self.receptor_structure_dms_file_name} --custom-protonation-states'
            else:
                fep_fixer_command = f'fepfixer -i {self.receptor_file_name} -o {self.receptor_structure_dms_file_name}'

            unitop_command = f'utop prm -i {self.receptor_structure_dms_file_name} -o {self.receptor_parameterized_dms_file_name}'

            os.system(fep_fixer_command)
            os.system(unitop_command)
        else:
            receptor_topology_preparation = ReceptorTopologyPreparation(self.receptor_file_name, self.working_dir_name)
    ...
    def prepare_covalent_bond_on_residue(self):
        ...
        covalent_residue_mol = self.protein_residue_property_mol_list[self.covalent_residue_idx]
        ...
        processed_covalent_residue_mol = get_mol_without_indices(covalent_residue_mol, remove_indices=self.covalent_residue_atom_idx_list, keep_properties=['atom_idx',
                                                                                                                                                            'atom_name',
                                                                                                                                                            'atom_charge',
                                                                                                                                                            'ff_atom_type',
                                                                                                                                                            'vina_atom_type',
        ...
        self.protein_residue_property_mol_list[self.covalent_residue_idx] = processed_covalent_residue_mol
    ...
    def generate_receptor_topology(self):
        ...
        if receptor_file_extension == 'pdb':
            ...
            raise ValueError('Only PDB and DMS are supported for receptor file extensions!!')

        receptor_msys_system = msys.LoadDMS(self.receptor_parameterized_dms_file_name)
        self.protein_property_mol, self.protein_residue_property_mol_list, self.cofactor_residue_property_mol_list = prepare_receptor_residue_mol_list(receptor_msys_system)
        ...
        for protein_residue_idx in range(num_protein_residues):
            protein_residue_property_mol = self.protein_residue_property_mol_list[protein_residue_idx]
        ...
        for cofactor_residue_idx in range(num_cofactor_residues):
            cofactor_residue_property_mol = self.cofactor_residue_property_mol_list[cofactor_residue_idx]
