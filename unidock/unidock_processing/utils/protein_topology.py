...
def prepare_protein_residue_mol_list(protein_pdb_file_name):
    ...
    original_protein_ag = mda.Universe(protein_pdb_file_name).select_atoms('protein or resname TPO')
...
def clean_pdb_file(protein_pdb_file_name, protein_cleaned_pdb_file_name):
    full_ag = mda.Universe(protein_pdb_file_name).atoms
    protein_ag = full_ag.select_atoms('protein')
    not_protein_ag = full_ag.select_atoms('not protein')
    atom_list = []
    for atom in protein_ag:
        if 'HX' in atom.name or atom.name == 'H2':
            continue
        if atom.name == 'H1':
            atom.name = 'H'
        if atom.name == 'H' and atom.resname == 'PRO':
            continue
        if atom.name == 'HD2' and atom.resname == 'ASN':
            atom.name = 'HD21'
    ...
    for atom in not_protein_ag:
        if len(atom.altLoc) != 0:
            if atom.altLoc == 'A':
                ...
                continue
        atom_list.append(atom)
    cleaned_ag = mda.AtomGroup(atom_list)
    cleaned_ag.write(protein_cleaned_pdb_file_name)
